...
Reset_Handler:  
	...
	// Last reason for reset was ?
	ldr     r0, RCC_FLAG_SFTRST
	ldr.w   r0, [r0,#0xD0]
	tst.w   r0, #0x1000000
	BEQ     boot_into_loader

	// jump to application(but maybe check if there is valid ptr/image there ?)
  	ldr		r0, FIRMWARE_ADDR
 	ldr		r0, [r0,#4]
  	bx		r0

RCC_FLAG_SFTRST:   .word 0x58024400
FIRMWARE_ADDR:	   .word 0x08020000
...
LoopFillZerobss:
	...
	// we don't want to use this, but still need spritnf() ;(
